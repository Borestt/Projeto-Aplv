...
function Avatar({ seed, onClick }) { // API Dicebear para gerar avatares
...
function CreateCharacterScreen() { // Tela de Criação de Personagem
  ...
  const gerarOpcoes = () => { // GeraR 4 novas opções de avatar
    const novasOpcoes = Array.from({ length: 4 }, () => Math.random().toString(36).substring(7));
    setOpcoesAvatar(novasOpcoes);
  };

  const rolarAtributos = () => { // Rola os atributos baseado na classe escolhida
  ...
  return (
    <div className="create-character-container">
      ...
      {avatarEscolhido && ( // Mostra o avatar escolhido
        <div id="resultado">
      ...
      <Link to="/game" onClick={() => { // Salva os dados do personagem no localStorage
